...
void loopcode(int n, double mass, double fcon, int delta, double grav,
  ...
  for (i = 0; i < n; i++) {
    for (j = 0; j < n; j++) {
      ...
      fvx = xdiff * rball / vmag;
      fvy = ydiff * rball / vmag;
      fvz = zdiff * rball / vmag;
      ...
      if (vmag < rball) {
        ...
        z[j * n + i] = zball + fvz;
        // update velocity
        vfdot = (fvx * vx[j * n + i] + fvy * vy[j * n + i] + fvz * vz[j * n + i]) / fmag / fmag;
        vx[j * n + i] = 0.1 * (vx[j * n + i] - vfdot * fvx);
        vy[j * n + i] = 0.1 * (vy[j * n + i] - vfdot * fvy);
        vz[j * n + i] = 0.1 * (vz[j * n + i] - vfdot * fvz);
      }
    }
  }
