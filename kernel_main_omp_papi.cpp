...
int main(int argc, char **argv) {
  ...
  for (iter = 0; iter < maxiter; iter++) {
    loopcode(n, mass, fcon, delta, grav, sep, rball, xball, yball, zball, dt, x,
             y, z, fx, fy, fz, vx, vy, vz, oldfx, oldfy, oldfz, &pe, &ke, &te, tn);
  }
  stop = omp_get_wtime();
  PAPI_stop(EventSet, counters);
  printf("%d,%lf,%lf,%lf,%d,%lf,%lf,%lf,%lf,%d,%lf,", n, sep, mass, fcon, delta, grav, rball, offset, dt, maxiter, stop - start);
  for (int i = 0; i < NUM; i++) {
